...
class SafetyMetrics:
    ...
    InstantaneousFast = 'InstantaneousFast'
...
#
# #        if (ix == 0):
# #            oneToTwoFootprint = ceb.PyFootprint(skygrid=curSkyGrid)     # create the footprint
# #        else:
...
def PlotSubEnvelopes(curMission, tfailSec):
    ...
    curFP = ceb.PyFootprint(footprintFileName=infileStr)
...
def makeFootprintFromTimes_InstantaneousOnly(mission1, timelo, timehi):
    ...
    try:
        ...
        totalFootPrint = ceb.PyFootprint(footprintFileName=fileNames[0])
        print fileNames[0]

        for curFile in fileNames[1:]:
            curFootPrint = ceb.PyFootprint(footprintFileName=curFile)
...
#         # debugFootprint = ceb.PyFootprint(footprintFileName=outfileStr)
...
def genFootprint(curMission, tfailSec, curPFail):
    ...
    myFootprint = PyFootprint(skygrid=curSkyGrid)
...
def getProbImpacts(curMission, tfailSec):
    ...
    if safetyMetric == SafetyMetrics.Cumulative:
        ...
        ans = P_RH.getGrid(), P_H.getGrid()
        # TODO: I'm enforcing deltaTFail = 1 already, so make delta_R and delta_H just be in seconds.
    
    #TODO: The GenerateSpatialProbability functions take integers, not doubles.  Fix that.
    elif safetyMetric == SafetyMetrics.InstantaneousFast:
        print "  time range {0}-{1}".format(tfailSec, tfailSec + delta_R + delta_H)
        ...
        # P_Imp = curSkyGrid.GenerateSpatialProbability_InstantaneousMax(ProbabilityCodes.IMPACT    , tfailSec + delta_R + delta_H, tfailSec)
...
def GenerateHazardVectorFiles(curMission, timeRange, pFailThisTimestepVec):
    ...
    elif curMission['safetyMetric'] == SafetyMetrics.InstantaneousFast:
        GenerateHazardVectorFiles_InstantaneousFast(curMission, timeRange, pFailThisTimestepVec)
...
def GenerateHazardVectorFiles_Instantaneous(curMission, timeRange, pFailThisTimestepVec):
    ...
    for tx in range(len(timeRange)):
        ...
        catastropeAtTime[tx] = dict() # Making each timestep its own dictionary

        timeKeys = sorted(P_Cas_Dict.keys())  # These should be the same keys for catastrophe as well.
        for curTimeKey in timeKeys:
            ...
            P_Cat = PyGrid3D(dict=P_Cat_Dict[curTimeKey]) * curPFail
            catastropeAtTime[tx][curTimeKey] = P_Cat

        del P_Cas_Dict # Get rid of this
        del P_Cat_Dict # Get rid of this
        ...
        for curTimeKey in timeKeys:
            ...
            P_Cat = catastropeAtTime[tx][curTimeKey]
            ...
            for prev_tx in range(max(tx-delta_H,0), tx, int(deltaT)):  # This won't include tx, which is find because we just initialized with it.
                if casualtyAtTime[prev_tx].has_key(curTimeKey):
                    ...
                    P_Cat += catastropeAtTime[prev_tx][curTimeKey]
            ...
            maxCas = skyCas.applyCumulativeThreshold(P_Cas, casThresh, np.array([int(tfailSec/deltaTFail)]))     
            curCasualtyFP = PyFootprint(skygrid=skyCas, armLength=armLength)

            skyCat = PySkyGrid(curMission=curMission)
            maxCat = skyCat.applyCumulativeThreshold(P_Cat, catThresh, np.array([int(tfailSec/deltaTFail)]))
            curCatastropheFP = PyFootprint(skygrid=skyCat, armLength=armLength)
            ...
            if not hasCasualtyFP:
                ...
                totalCatastropheFP = PyFootprint(skygrid=skyCat, armLength=armLength)
                hasCasualtyFP = True
            else:
                totalCasualtyFP.MergeFootprintVectors(curCasualtyFP)
                totalCatastropheFP.MergeFootprintVectors(curCatastropheFP)
        ...
        if casualtyAtTime.has_key(tx-delta_H):
            ...
            del catastropeAtTime[tx-delta_H]

        # All those merges together can mean a lot of repeated points.  Smooth it out.
        totalCasualtyFP.SmoothedOut(newDeltaT=curMission['deltaT'], armLength=armLength)
        totalCatastropheFP.SmoothedOut(newDeltaT=curMission['deltaT'], armLength=armLength)
        # print "WARNING: The timestep here should be deltaT!  Using a deltaTFail value now for debugging purposes."
        ...
        totalCatastropheFP.StoreFootprintAsVector(outfileStrCat)
        ...
        # totalCatastropheFP.ExportGoogleEarth(outfileStrCat, yyyy, mm, dd, hour, min)
        ...
        totalCatastropheFP.MergeFootprintVectors(totalCasualtyFP)    # Do the merge
        totalCatastropheFP.StoreFootprintAsVector(outfileStr)
...
def GenerateHazardVectorFiles_InstantaneousFast(curMission, timeRange, pFailThisTimestepVec):
    print "In progress"
    armLength                   = curMission['armLength']
    footprintUntil              = timeRange[-1]
...
def GenerateHazardVectorFiles_Cumulative(curMission, timeRange, pFailThisTimestepVec):
    ...
    for tx in range(len(timeRange)):
        ...
        ## Turn off the hazard areas where the danger is completely passed
        ...
        tfailSec += deltaTFail   # TODO: This isn't necessary, right?!
