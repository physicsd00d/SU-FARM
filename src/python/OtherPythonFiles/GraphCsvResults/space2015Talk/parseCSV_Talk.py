...
topToBottomTalk = ["SS2_America_2018L",
                   "Lynx_FrntRnge_2018H",
                   "Atlas5_Vafb_2018M",
                   "Antares_Wallops_2018M"]
...
outputPath="/Users/marian/Desktop/AIAA Space2015 Talk_Tex/Figures/"
...
plotDict = dict(figsize             = (26, 5),   # x, y
...
topToBottomIndividual = []
topToBottomIndividual.extend(topToBottomTalk)
ax1, ax2, ax3, ax4, f = PlotTest(tradMissions, sc2tl, topToBottomIndividual, plotDict, outputPath, outputFile)

#%% Pull out the values so I can make a table

# This works for no confidence intervals
for missionKey in topToBottomIndividual:
    subAvg = tradMissions[missionKey]['average']
    subConf = tradMissions[missionKey]['confidence']
    newLine = sc2tl[missionKey].find('\n')
    tableValues = ["{0:20}".format(sc2tl[missionKey][:newLine-1]),
                  "{0:<12.2f}".format(subAvg['rerouted']),
                  "{0:<12.2f}".format(subAvg['numFilter']),
                  "{0:<12.2f}".format(subAvg['DeltaTime']),
                  "{0:<12.2f}".format(subAvg['sumFilterMinutes']),
                  "{0:<12.2f}".format(subAvg['DeltaFuel']),
                  "{0:<12.2f}".format(subAvg['DeltaDist'])]
    print "&".join(tableValues) + "\\\\" + "\n"
...
#    newLine = sc2tl[missionKey].find('\n')
#    tableValues = ["{0:20}".format(sc2tl[missionKey][:newLine-1]),
#                  "{0:<20}".format(  "${0:.2f}\pm{1:.2f}$".format(subAvg['rerouted'], subConf['rerouted'])  )     ,
#                  "{0:<20}".format(  "${0:.2f}\pm{1:.2f}$".format(subAvg['numFilter'], subConf['numFilter'])  )     ,
#                  "{0:<20}".format(  "${0:.2f}\pm{1:.2f}$".format(subAvg['DeltaTime'], subConf['DeltaTime'])  )     ,
#                  "{0:<20}".format(  "${0:.2f}\pm{1:.2f}$".format(subAvg['sumFilterMinutes'], subConf['sumFilterMinutes'])  )     ,
#                  "{0:<20}".format(  "${0:.2f}\pm{1:.2f}$".format(subAvg['DeltaFuel'], subConf['DeltaFuel'])  )     ,
#                  "{0:<20}".format(  "${0:.2f}\pm{1:.2f}$".format(subAvg['DeltaDist'], subConf['DeltaDist'])  )]
#    print "&".join(tableValues) + "\\\\" + "\n"
...
# This works for no confidence intervals
for missionKey in topToBottomIndividual:
    subAvg = envMissions[missionKey]['maximum']
    newLine = sc2tl[missionKey].find('\n')
    tableValues = ["{0:20}".format(sc2tl[missionKey][:newLine-1]),
                  "{0:<12.2f}".format(subAvg['rerouted']),
                  "{0:<12.2f}".format(subAvg['numFilter']),
